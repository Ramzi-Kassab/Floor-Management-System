...
class CustomerAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'is_billing_entity', 'is_operating_entity', 'city', 'country'
    ]
    list_filter = [
        'customer_type', 'account_status', 'country',
        'is_billing_entity', 'is_operating_entity',
        'requires_coc', 'requires_dull_grade'
    ]
    search_fields = [
        'customer_code', 'name', 'legal_name', 'external_id',
        'primary_contact_email'
    ]
    readonly_fields = ['public_id', 'created_at', 'updated_at']
    fieldsets = (
        ('Identification', {
            'fields': ('customer_code', 'name', 'legal_name', 'external_id', 'public_id')
        }),
        ('Classification', {
            'fields': (
...
class RigAdmin(admin.ModelAdmin):
    list_display = [
        ...
        'owner_customer', 'current_location', 'max_depth_ft'
    ]
    list_filter = ['rig_type', 'status', 'top_drive']
...
class SalesOrderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Customer', {
            'fields': ('customer', 'billing_customer', 'well', 'rig', 'delivery_location')
        }),
        ('Dates', {
            'fields': (
        ...
        ('Financial', {
            'fields': ('total_value', 'currency', 'payment_terms_days')
        }),
        ('Production Link', {
        ...
        ('Requirements', {
            'fields': ('special_instructions', 'quality_requirements')
        }),
        ('Invoice', {
    ...
    fieldsets = (
        ('Identification', {
            'fields': (
                ...
                'mat_number', 'bit_size_inches', 'run_sequence', 'public_id'
            )
        }),
        ('Location', {
            'fields': ('customer', 'well', 'rig', 'hole_section', 'hole_type')
        }),
        ('Depth & Performance', {
            'fields': (
        ...
        ('Mud Properties', {
            'fields': (
                'mud_type', 'mud_weight_ppg', 'mud_properties_json'
            )
        }),
        ('Nozzles', {
            'fields': ('nozzle_configuration', 'tfa_sqin')
        }),
        ('Formation', {
            'fields': ('formation_name', 'lithology')
        }),
        ('Problems & Trip', {
            'fields': ('problems_encountered', 'downhole_tool_failures', 'trip_reason')
        }),
        ('Status & Timing', {
        ...
        ('Additional Data', {
            'fields': (
                'bha_details_json', 'directional_data_json', 'operational_summary'
            ),
            'classes': ('collapse',)
        }),
        ('Reporting', {
            'fields': ('reported_by', 'report_date', 'dull_grade_id')
        }),
        ('Notes', {
            'fields': ('notes',)
        }),
